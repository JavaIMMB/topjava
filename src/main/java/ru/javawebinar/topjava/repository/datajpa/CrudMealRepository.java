...
public interface CrudMealRepository extends JpaRepository<Meal, Integer> {
    ...
    List<Meal> deleteAll(@Param("userId") int userId);

    @Query("SELECT m FROM Meal m WHERE m.user.id=:userId AND m.dateTime >= :after AND m.dateTime < :before ORDER BY m.dateTime DESC")
    List<Meal> getBetween(@Param("after")LocalDateTime startDate, @Param("before")LocalDateTime endDate, @Param("userId")int userId);
}
